...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ,'<div class="layui-layim-info">'
      ,'<img class="img-circle pull-left" id="indexIco" style="width: 40px;" src="{{ d.mine.avatar }}">'
      ,'<div class="layui-layim-user" style="margin-left: 10px;">{{ d.mine.username }}</div>'
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ,'<div class="layui-unselect layim-chat-title">'
      ,'<div class="layim-chat-other">'
        ...
        ,'<p class="layim-chat-status">最近上线 {{ d.data.difftime }} 前</p>'
      ,'</div>'
    ,'</div>'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layui-unselect layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ...
        ,'{{# }; }}'
        ,'<span style="font-size: 12px;" layim-event="clearLog">清空对话</span>'
      ,'</div>'
      ,'<div class="layim-chat-textarea" id="editDiv" contenteditable="true"><textarea></textarea></div>'
  ...
  //聊天内容列表模版
  var elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine" : "class=layim-chat-to" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>'
  ...
  var sendMessage = function(){
    ...
    var data = {
      ...
      ,difftime: cache.mine ? cache.mine.difftime : '刚刚'
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    // 清空对话
    ,clearLog: function(othis){
        $(".layim-chat-mine").remove();//清除自己本地缓存
        $(".layim-chat-to").remove();//清除对方本地缓存
